...
When('o usuário preenche o campo {string} com {string} e clica no botão {string}', (field: string, value: string, button: string) => {
...
When('o usuário não preenche o campo {string} e clica no botão {string}', (_: string, button: string) => {
  cy.getDataCy(button).click()
})
...
Given('o usuário está na página {string} com o test {string} criado', (page: string) => {
  cy.visit(page)
})
...
Then('o usuário deve ir para a página {string} e ver o test {string}', (page: string, test: string) => {
