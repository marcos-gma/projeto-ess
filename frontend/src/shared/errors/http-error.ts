...
abstract class HttpError extends AppError {
  ...
  protected constructor(slug: string, message: string, stackTrace: string, statusCode = -1) {
  ...
  static parseHttpError(error: AxiosError) {
    ...
    if (error.response?.data) {
      const baseApiResponseModel = new BaseApiResponseModel(error.response?.data as any)
...
class HttpUnknownError extends HttpError {
  constructor({ slug = 'unknown', message = 'Unknown error', stackTrace = '' }) {
    super(slug, message, stackTrace)
  }
}

class HttpBadRequestError extends HttpError {
  constructor({ slug = 'bad_request', message = 'Bad request', stackTrace = '' }) {
    super(slug, message, stackTrace, 400)
  }
}

class HttpUnauthorizedError extends HttpError {
  constructor({ slug = 'unauthorized', message = 'Unauthorized', stackTrace = '' }) {
...
class HttpInternalServerError extends HttpError {
  constructor({ slug = 'internal_server_error', message = 'Internal server error', stackTrace = '' }) {
